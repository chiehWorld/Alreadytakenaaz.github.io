---
layout:     post
title:      分类算法之决策树(Decision tree)
date:       2026-01-22
author:     Chieh
header-img: assets/算法杂货铺——分类算法之决策树(Decision-tree)_header.png
catalog: true
tags:
    - 数据挖掘
    - 算法
...
相比贝叶斯算法，决策树的优势在于构造过程不需要任何领域知识或参数设置，因此在实际应用中，对于探测式的知识发现，决策树更加适用。在这一篇文章中，将讨论另一种被广泛使用的分类算法—— [决策树](http://en.wikipedia.org/wiki/Decision_tree)（decision tree）。

**相关链接**：

[朴素贝叶斯分类](http://en.wikipedia.org/wiki/Naive_Bayes_classifier)

[贝叶斯网络](http://en.wikipedia.org/wiki/Bayesian_network)

这两种算法都以 [贝叶斯定理](http://en.wikipedia.org/wiki/Bayesian_network) 为基础，可以对分类及决策问题进行概率推断。
...
![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_1.png)
...
构造决策树的关键性内容是进行属性选择度量，属性选择度量是一种选择分裂准则，是将给定的类标记的训练集合的数据划分D“最好”地分成个体类的 [启发式方法](http://en.wikipedia.org/wiki/Heuristic_(computer_science))，它决定了拓扑结构及分裂点split\_point的选择。
...
![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_2.jpg "info(D)=-\sum ^m_{i=1}p_ilog_2(p_i)")

其中pi表示第i个类别在整个训练元组中出现的概率，可以用属于此类别元素的数量除以训练元组元素总数量作为估计。熵的实际意义表示是D中元组的类标号所需要的平均信息量。

现在我们假设将训练元组D按属性A进行划分，则A对D划分的期望信息为：

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_3.jpg "info_A(D)=\sum ^v_{j=1}\frac{|D_j|}{|D|}info(D_j)")

而信息增益即为两者的差值：

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_4.jpg "gain(A)=info(D)-info_A(D)")

ID3算法就是在每次需要分裂时，计算每个属性的增益率，然后选择增益率最大的属性进行分裂。下面我们继续用SNS社区中不真实账号检测的例子说明如何使用ID3算法构造决策树。为了简单起见，我们假设训练集合包含10个元素：

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_5.png)

其中s、m和l分别表示小、中和大。

设L、F、H和R表示日志密度、好友密度、是否使用真实头像和账号是否真实，下面计算各属性的信息增益。

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_6.jpg "info(D)=-0.7log_20.7-0.3log_20.3=0.7*0.51+0.3*1.74=0.879")

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_7.jpg "info_L(D)=0.3*(-\frac{0}{3}log_2\frac{0}{3}-\frac{3}{3}log_2\frac{3}{3})+0.4*(-\frac{1}{4}log_2\frac{1}{4}-\frac{3}{4}log_2\frac{3}{4})+0.3*(-\frac{1}{3}log_2\frac{1}{3}-\frac{2}{3}log_2\frac{2}{3})=0+0.326+0.277=0.603")

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_8.jpg "gain(L)=0.879-0.603=0.276")
...
![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_9.png)
...
![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_10.jpg "split\_info_A(D)=-\sum ^v_{j=1}\frac{|D_j|}{|D|}log_2(\frac{|D_j|}{|D|})")

其中各符号意义与ID3算法相同，然后，增益率被定义为：

![](assets/算法杂货铺——分类算法之决策树(Decision-tree)_img_11.jpg "gain\_ratio(A)=\frac{gain(A)}{split\_info(A)}")
...
---
> 参考链接：[https://www.cnblogs.com/leoo2sk/archive/2010/09/19/decision-tree.html](https://www.cnblogs.com/leoo2sk/archive/2010/09/19/decision-tree.html)
